...
class ExecutionNodeHostTests(unittest.TestCase):
    ...
    def test_linux_service_lifecycle_is_explicit_reversible_and_non_destructive(self):
        install = (ROOT / ".adwf/runtime-host/linux/install-user-service.sh").read_text(encoding="utf-8")
        self.assertIn("install|verify|disable", install)
        self.assertIn('if [ "$ACTION" = "verify" ]', install)
        self.assertIn('if [ "$ACTION" = "disable" ]', install)
        self.assertIn('systemctl --user disable --now "$SERVICE"', install)
        self.assertIn("token, repository, logs, and unit file were preserved", install)
        self.assertNotIn("rm -f \"$TOKEN_FILE\"", install)
        self.assertNotIn("git reset --hard", install)
        self.assertNotIn("git clean", install)
